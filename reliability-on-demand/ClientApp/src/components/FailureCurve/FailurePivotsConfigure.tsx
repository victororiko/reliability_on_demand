...
export class FailurePivotsConfigure extends React.Component<
  IFailurePivotsConfigureProps,
  IFailurePivotsConfigureState
> {
  cols: IColumn[] = []
  ...
  async populateData() {
    await axios
      .get(
        `api/Data/GetAllailurePivotNamesForAVertical/${this.props.selectedVerticalForStudy.key}`
      )
      .then((res) => {
  ...
  getDefaultPivotKeys() {
    if (this.resultantPivotSQL)
      ...
      console.log(
        `resultantPivotSQL is Empty i.e. could not get default pivots for selected vertical`
      )
  }
  ...
  onSelectingPivot = (
    ...
    if (item) {
      let keyUpdated = item.selected
        ? [...(this.state.selectedPivotsOnlyKey ?? []), item.key as number]
        : this.state.selectedPivotsOnlyKey?.filter((val) => val !== item.key)
  ...
  mapPivotTableColumnValue(
    ...
    switch (colname) {
      case 'IsSelectPivot':
        arr[row].IsSelectPivot = val
        break
      case 'IsKeyPivot':
        arr[row].IsKeyPivot = val
        break
      case 'IsApportionPivot':
        arr[row].IsApportionPivot = val
        break
      case 'IsApportionJoinPivot':
        arr[row].IsApportionJoinPivot = val
        break
      case 'IsScopeFilter':
        arr[row].IsScopeFilter = val
        break
      case 'FilterExpression':
        arr[row].FilterExpression = val
        break
      case 'FilterExpressionOperator':
        arr[row].FilterExpressionOperator = val
        break
      default:
        break
    }
  ...
  nextClicked() {
    ...
    return (
      <div>
        <FailureCurveFilterExpression
          failureConfigToSave={failureObjToBePassed}
        />
      </div>
    )
  }
  ...
  async onPivotDropdownLoad() {
    ...
    if (
      this.resultantPivotSQL === null ||
      this.resultantPivotSQL.length === 0
    ) {
      await axios
        .post('api/Data/GetAllDefaultFailurePivotsForAVertical', input, {
          headers: {
    ...
    let pivottable = (
      <div>
        <WikiLink
          title={'Watson Wiki'}
          url={
            'https://www.osgwiki.com/wiki/Watson_1508_SnapShot_API#WatsonSnapshotAggView_1508_parameters'
          }
        />
      ...
      <div>
        <TooltipHost content="Select/Deselect Pivots based on if it is part of failure curve">
          <Dropdown
            ...
            multiSelect
            options={this.getPivotNames(this.state.pivotsList)}
