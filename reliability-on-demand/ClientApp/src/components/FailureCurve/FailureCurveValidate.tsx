...
export class FailureCurveValidate extends React.Component<
  IFailureCurveValidateProps,
  IFailureCurveValidateState
> {
  ...
  async _validateClicked() {
    ...
    if (flag == false) {
      this.validateStatement =
        'Filter expression has been validated successfully' +
        '<br/>' +
        this.showFilterExpression()
  ...
  showFilterExpression() {
    ...
    for (let ele of this.props.FilterExpArr) {
      if (ele.RelationalOperator != null && ele.RelationalOperator != '') {
        ...
        lastPieceInExp =
          ele.PivotName + ' ' + ele.Operator + ' ' + ele.PivotValue
      }
    }
  ...
  render() {
    ...
    let validateStatement = (
      <div>
        <Label>{this.validateStatement}</Label>
      </div>
    )
    return (
      <div>
