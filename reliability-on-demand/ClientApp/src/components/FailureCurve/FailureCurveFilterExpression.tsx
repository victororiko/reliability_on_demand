...
export class FailureCurveFilterExpression extends React.Component<
  IFailureCurveFilterExpressionProps,
  IFailureCurveFilterExpressionState
> {
        ...
        if (exp != null && exp != '') {
          for (let eleExp of exp.split(ele.PivotName)) {
            if (eleExp != null && eleExp != '') {
              ...
              var rop = this.getContainingElementFromArr(
                eleExp,
                this.state.RelationalOperators
              )
              ...
              for (let eleRop of ropArr) {
                ...
                if (trimele != null && trimele != '') {
                  ...
                  var op = this.getContainingElementFromArr(
                    trimele,
  ...
  renderSaveButton() {
      ...
      for (var i = 0; i < pivotexpMap.length; i++) {
        ...
        if (pid == id) {
          ...
          var exp =
            filterexp +
            ' ' +
            pivotexpMap[i].RelationalOperator +
            ' ' +
            d.PivotName +
            ' ' +
            d.Operator +
    ...
    return (
      <div>
        <FailureCurveSave
          failureConfigToSave={this.props.failureConfigToSave}
        />
      </div>
    )
  }
  ...
  renderDetailedList() {
    return (
      <div>
        <TooltipHost content="Configure the filter expression">
          <DetailsList
  ...
  addClicked(id: any) {
    ...
    this.DefaultPivot = [
      ...this.DefaultPivot.slice(0, id),
      item,
      ...this.DefaultPivot.slice(id),
    ]
  ...
  mapPivotTableColumnValue(
    ...
    switch (colname) {
      case 'Operator':
        arr[row].Operator = val
        break
      case 'PivotID':
        arr[row].PivotID = val
        break
      case 'PivotName':
        arr[row].PivotName = val
        break
      case 'PivotScopeID':
        arr[row].PivotScopeID = val
        break
      case 'PivotValue':
        arr[row].PivotValue = val
        break
      case 'RelationalOperator':
        arr[row].RelationalOperator = val
        break
      default:
        break
    }
      ...
      return (
        <span>
          <TextField
            ...
            placeholder={this.pivotValuePlaceholder}
          />
        </span>
      )
  }
  ...
  render(): React.ReactElement {
    let nextButton = (
      <div>
        <TooltipHost content="Click Next to configure the filter expression">
          <DefaultButton
    ...
    let detailedList =
      this.HasNextClicked == true ? this.renderDetailedList() : ''
    ...
    let vaildateButton =
      this.HasNextClicked == true ? this.renderValidate() : ''
