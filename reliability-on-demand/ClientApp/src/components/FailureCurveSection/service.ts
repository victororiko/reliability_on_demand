...
export const extractModesFromVerticalPair = (verticals: IDropdownOption[]): IDropdownOption[] => {
...
export const getMappedPivotWithScopeFilter = (input: Pivot[], studyconfigID: number): Pivot[] => {
    const res: Pivot[] = []
...
export const getContainingElementFromArr = (input: string, arr: IDropdownOption[]): string => {
    let res: string = ""
    for (const op of arr) {
