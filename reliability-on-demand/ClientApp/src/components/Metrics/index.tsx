...
export const Metrics = (props: Props) => {
    ...
    const loadMetrics = async (StudyConfigID: number = CreateNewID) => {
        ...
        try {
            ...
            const response = await axios.get(`api/Data/GetDefaultMetricsConfig/${StudyConfigID}`)
            let defaultsFromBackend: Metric[] = []
            if (response && response.data !== "") defaultsFromBackend = response.data as Metric[]
            // get user metrics from backend
            const response2 = await axios.get(`api/Data/GetMetricConfigs/${StudyConfigID}`)
