...
export const getPivotMap = (
    ...
    for (const ele of input) {
        if (map.has(ele.PivotKey)) {
            ...
            let arr: IDropdownOption[] = [{ key: "Add new value", text: "Add new value" }]
            ...
            const arr: IDropdownOption[] = [{ key: "Add new value", text: "Add new value" }]
            arr?.push({ key: ele.PivotValue, text: ele.PivotValue })
...
export const getPivotScopeIDs = (input: StudyPivotConfig[]): StudyConfigIDWithScopesInquiry => {
...
export const getPivotNamePairs = (input: FilterExpTable[]): IDropdownOption[] => {
...
export const getAllFilteredPivots = (input: StudyPivotConfig[]): IDropdownOption[] => {
...
export const convertStudyPivotToFilterExp = (input: StudyPivotConfig[]): FilterExpTable[] => {
    const res: FilterExpTable[] = []
...
export const getRelationalOperatorCount = (input: StudyPivotConfig[]): number => {
    let ropCount = 0
    for (const ele of input) {
        if (
            ele.RelationalOperator &&
            ele.RelationalOperator !== null &&
            ele.RelationalOperator !== ""
        ) {
            ropCount += 1
        }
...
export const showFilterExpression = (input: StudyPivotConfig[]) => {
    let filterexp = ""
    let lastPieceInExp = ""
